...
func wrapEmbeddingUsage(e Embedder, config Config) Embedder {
	if e == nil {
		return e
	}
	return &usageEmbedder{inner: e, config: config}
}
...
func (w *usageEmbedder) BatchEmbed(ctx context.Context, texts []string) ([][]float32, error) {
	...
	w.record(ctx, span, start, len(texts), err)
	return vecs, err
}
...
func (w *usageEmbedder) record(ctx context.Context, span *usageSpan, start time.Time, inputs int, err error) {
